...
public class ServoInterpolator {
    ...
    public ServoInterpolator(Servo s) {
        servo = s;
        setServoRange(0.75);
    ...
    public ServoInterpolator setConstraints(double vel, double accel, double prop) {
        maxVel = vel;
        maxAccel = accel;
        proportion = prop;
        return this;
    }
    public ServoInterpolator setConstraints(Constraints c) {
        return setConstraints(c.maxVelocity, c.maxAcceleration, c.proportion);
    }

    public ServoInterpolator setServoRange(double rangeInRotations) {
        servoRange = rangeInRotations;
        return this;
    }

    public ServoInterpolator setTargetPosition(double target) {
        targetPosition = target;
        deltaTime.reset();
        return this;
    }

    public ServoInterpolator setTargetTolerance(double tolerance) {
        targetTolerance = tolerance;
        return this;
    }
    //fun method to update servo
    public ServoInterpolator update() {
