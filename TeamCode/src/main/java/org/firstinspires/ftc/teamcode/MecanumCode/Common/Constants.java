package org.firstinspires.ftc.teamcode.MecanumCode.Common;

public class Constants {
    // Game Constants
    public static final int TOP_GOAL_POS = 1560;
    public static final int MID_GOAL_POS = 730;
    public static final int BOTTOM_GOAL_POS = 0;
    public static final int CAPPING_POS = 2650;

    public static final int MAGARM_EXTENDED = -275;
    public static final int MAGARM_RETRACTED = -100;

    public static final double LEVEL_DOWN_POS = 0;
    public static final double LEVEL_UP_POS = 1;
}
