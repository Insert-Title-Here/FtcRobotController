...
@TeleOp (name = "PhotonTesting")
public class PhotonTeleOpTesting extends LinearOpMode {


    Constants constants = new Constants();
    ScoringSystemPhoton score;
    MecDrive drive;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        score = new ScoringSystemPhoton(hardwareMap, constants);
            ...
            public void run() {
                while(opModeIsActive()){
                    ...
                    if(gamepad1.right_trigger > 0.1){
                        ...
                        if(score.getScoringMode() == ScoringSystemPhoton.ScoringMode.LOW && score.isExtended()) {
                            try {
                                sleep(500);
                    ...
                    if((distance.getNormalizedColors().red > 0.85 || distance.getNormalizedColors().blue > 0.85) && autoLinkageFlag){
                        ...
                        if(score.getScoringMode() == ScoringSystemPhoton.ScoringMode.ULTRA){
                            score.autoGoToPosition();
                            score.setPower(0.2);
                            try {
                                Thread.sleep(100);
                    ...
                    if(gamepad1.y){
                        score.setScoringMode(ScoringSystemPhoton.ScoringMode.LOW);

                    }else if(gamepad1.x){
                        score.setScoringMode(ScoringSystemPhoton.ScoringMode.MEDIUM);

                    }else if(gamepad1.b){
                        score.setScoringMode(ScoringSystemPhoton.ScoringMode.HIGH);

                    }else if(gamepad1.a){
                        //Ultra
                        score.setScoringMode(ScoringSystemPhoton.ScoringMode.ULTRA);
                    }
